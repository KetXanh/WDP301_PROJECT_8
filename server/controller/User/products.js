  ...
  try {
    ...
    const formatProduct = products
      .filter(
        (v) =>
          v.baseProduct &&
          v.baseProduct.subCategory &&
          v.baseProduct.subCategory.category
      )
      .map((p) => ({
        _id: p._id,
...
module.exports.detailProducts = async (req, res) => {
  try {
    ...
    const product = productV.find(
      (p) => p.baseProduct?.slug.toLowerCase() === slug.toLowerCase()
    );
